...
from bulbs.neo4jserver import Config


class Neo4jGitHubMetadataStore(GitHubMetadataStore):
    """
    neo4j-backed GitHubMetadataStore
    ...
    def __init__(self, config, githubHelper):
        ...
        neoUri   = 'http://localhost:7474/db/data/'
        
        neoConfig   = Config(neoUri, username, password)
        
        self.graph  = Graph(neoConfig)

        # just one time? or every time we instantiate?
        # self.graph.add_proxy("people", Person)
